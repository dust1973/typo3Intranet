 ...
 */
class  tx_myquizpoll_module1 extends t3lib_SCbase {
	var $pageinfo;
	var $path;
...
//				'5' => $LANG->getLL('function5'),
	...
	function insertLine($names, $values, $Felder) {
		$result = '';
		$convert = intval(t3lib_div::_GP('convert'));
		$show_it = intval(t3lib_div::_GP('show_it'));
...
//			$this->doc->form.='<input type="hidden" name="id" value="'.t3lib_div::_GP('id').'" />'."\n";
			$this->doc->form.='<input type="hidden" name="delall" value="0" />'."\n";
			
			// Styles
			//$this->doc->styleSheetFile2=$GLOBALS["temp_modPath"].'style.css' ODER "../".substr(t3lib_extMgm::extPath($this->extName),strlen(PATH_site))."mod1/style.css";
			if (file_exists(t3lib_div::getIndpEnv('TYPO3_DOCUMENT_ROOT').'/uploads/tx_myquizpoll/style.css'))
			...
			$headerSection = $this->doc->getHeader('pages',$this->pageinfo,$this->pageinfo['_thePath']).'<br />'.$LANG->sL('LLL:EXT:lang/locallang_core.xml:labels.path').': '.t3lib_div::fixed_lgd_cs($this->pageinfo['_thePath'],55);
	...
	function moduleContent()	{
		...
		if (t3lib_div::_GP('delall')==1) {	// delete all rows
			...
			$content .= (t3lib_div::_GP('hide_some')) ? $LANG->getLL('hidden') : $LANG->getLL('released');
			$content .= '</strong></p><br /><br />';
		} else if (t3lib_div::_GP('user_id')) {
			$uid = intval(t3lib_div::_GP('user_id'));
			$detail = intval(t3lib_div::_GP('detail'));
			$userHash = array();
			...
			return;
		} else if (t3lib_div::_GP('mystyles')) {
			$datei = fopen($filename_new,"w+");
			fputs($datei, t3lib_div::_GP('mystyles'));
			fclose ($datei);
		} else if (t3lib_div::_GP('daten_id') && $_FILES['uploadfile'] && trim($_FILES['uploadfile']['name'])) {
			$daten_id = intval(t3lib_div::_GP('daten_id'));
			$lang_id = intval(t3lib_div::_GP('lang_id'));
			$show_it = intval(t3lib_div::_GP('show_it'));
			$feldtrenner = t3lib_div::_GP('feldtrenner');
			$texttrenner = t3lib_div::_GP('texttrenner');
			...
			//	$filename =  $GLOBALS["HTTP_SERVER_VARS"]["DOCUMENT_ROOT"]."/uploads/tx_myquizpoll/".$file["name"];
				$target_path = t3lib_div::getIndpEnv('TYPO3_DOCUMENT_ROOT') .'/uploads/tx_myquizpoll/import.csv';	//. basename( $file['name']);
			...
			case 7:
			  ...
			  if ($GLOBALS['BE_USER']->user['admin']) {
				...
				if (t3lib_div::_GP('export')) {
					...
					$feldtrenner = t3lib_div::_GP('feldtrenner');
					$texttrenner = t3lib_div::_GP('texttrenner');
					$dateformat =  t3lib_div::_GP('dateformat');
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_myquizpoll_module1');
$SOBE->init();
