 ...
 */
class Tx_WoehrlSeminare_Command_CheckeventsCommandController extends Tx_Extbase_MVC_Controller_CommandController {
	...
	*/
	 ...
	 *
	 * @param Tx_WoehrlSeminare_Domain_Repository_EventRepository $eventRepository
	 * @return void
	 */
	public function injectEventRepository(Tx_WoehrlSeminare_Domain_Repository_EventRepository $eventRepository) {
	 ...
	 *
	 * @param Tx_WoehrlSeminare_Domain_Repository_SubscriberRepository $subscriberRepository
	 * @return void
	 */
	public function injectSubscriberRepository(Tx_WoehrlSeminare_Domain_Repository_SubscriberRepository $subscriberRepository) {
    ...
    public function checkForSubscriptionEndCommand($storagePid, $senderEmailAddress = '') {
		...
		foreach($allevents as $event) {
			...
			if ($this->subscriberRepository->countAllByEvent($event) < $event->getMinSubscriber()) {
				...
				$out = $this->sendTemplateEmail(
					array($event->getContact()->getEmail() => $event->getContact()->getName()),
					array($senderEmailAddress => 'WÖHRL Akademie - noreply'),
					'Absage der Veranstaltung: ' . $event->getTitle(),
					'CancellEvent',
					array(	'event' => $event,
	...
	*/
	...
	protected function sendTemplateEmail(array $recipient, array $sender, $subject, $templateName, array $variables = array()) {
		...
		$templateRootPath = PATH_site . 'typo3conf/ext/woehrl_seminare/Resources/Private/Backend/Templates/';
		$partialRootPath = PATH_site . 'typo3conf/ext/woehrl_seminare/Resources/Private/Backend/Partials/';
		...
		if ($variables['attachIcs'] == TRUE) {
			...
			$eventIcsFile = PATH_site.'typo3temp/tx_woehrlseminare/'. preg_replace('/[^\w]/', '', $variables['helper']['nameto']).'-'. $templateName.'-'.$variables['event']->getUid().'.ics';
		...
		if ($variables['attachCsv'] == TRUE) {
			...
			$eventCsvFile = PATH_site.'typo3temp/tx_woehrlseminare/'. preg_replace('/[^\w]/', '', $variables['helper']['nameto']).'-'. strtolower($templateName).'.csv';
