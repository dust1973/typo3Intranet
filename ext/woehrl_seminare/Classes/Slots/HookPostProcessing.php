 ...
 */
class Tx_WoehrlSeminare_Slots_HookPostProcessing {

	/**
	 * Clear cache of all pages with woehrlseminare_eventlist plugin
	...
	function clearAllEventListCache($pid = 0, $isGeniusBar = 0) {
		...
		if ($isGeniusBar)
			$tcemain->clear_cacheCmd('cachetag:tx_woehrlseminare_cat_'.$pid);
		else
			$tcemain->clear_cacheCmd('cachetag:tx_woehrlseminare_'.$pid);
	...
	function processDatamap_afterDatabaseOperations($status, $table, $idElement, &$fieldArray, &$pObj) {
		...
		// we are only interested in tx_woehrlseminare_domain_model_event
		if ($table == 'tx_woehrlseminare_domain_model_event' &&
