...
class Tx_WoehrlSeminare_ViewHelpers_Be_FunctionBarViewHelper extends Tx_Fluid_ViewHelpers_Be_AbstractBackendViewHelper {
	...
	protected function getEditIcon($table, array $row){
		...
		$title = Tx_Extbase_Utility_Localization::translate('be.editEvent', 'woehrl_seminare', $arguments=NULL) . ' ' . $row['uid'] . ': ' . $row['title'] ;
	 ...
	 * @return string html output
	 */
	protected function getGeniusBarIcon(Tx_WoehrlSeminare_Domain_Model_Event $event){
	...
	public function render($icon = 'edit', Tx_WoehrlSeminare_Domain_Model_Event $event = NULL) {
		...
		switch ($icon) {
			case 'new': 	$content = $this->getNewIcon('tx_woehrlseminare_domain_model_event', $row);
				break;
			case 'edit': 	$content = $this->getEditIcon('tx_woehrlseminare_domain_model_event', $row);
				break;
			case 'hide': 	$content = $this->getHideIcon('tx_woehrlseminare_domain_model_event', $row);
				break;
