...
class ext_update {
	...
	public function access() {
		...
		$countMmTable = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'tx_woehrlseminare_event_discipline_mm', '1=1');

		$countGeniusBarEvent = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'tx_woehrlseminare_domain_model_event', 'genius_bar=1');
		$countGeniusBarCategory = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('*', 'tx_woehrlseminare_domain_model_category', 'genius_bar=1');
	...
	protected function updateContentRelationToMm() {
		...
		if ($countMmTable === 0) {
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,discipline', 'tx_woehrlseminare_domain_model_event', 'deleted=0');
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_woehrlseminare_domain_model_event', 'uid=' . $row['uid'], $update);
			}
			$GLOBALS['TYPO3_DB']->exec_INSERTmultipleRows('tx_woehrlseminare_event_discipline_mm', $fields, $inserts);
			...
			$this->messageArray[] = array(t3lib_FlashMessage::ERROR, $title, 'ERROR: table tx_woehrlseminare_event_discipline_mm. Did you run db compare after udpate?');
	...
	protected function updateCategoriesGeniusBar() {
		...
		if ($countGeniusBarEvent > 0 &&  $countGeniusBarCategory === 0) {
			...
			$categoryCount = 0;
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid_foreign', 'tx_woehrlseminare_domain_model_event, tx_woehrlseminare_event_category_mm', 'genius_bar=1 AND uid_local=uid');
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				$categoryCount++;

				// Update event record
				$update = array('genius_bar' => 1);
				$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_woehrlseminare_domain_model_category', 'uid=' . $row['uid_foreign'], $update);
			}
			$GLOBALS['TYPO3_DB']->sql_free_result($res);
			...
			$this->messageArray[] = array(t3lib_FlashMessage::ERROR, $title, 'ERROR: table tx_woehrlseminare_domain_model_category. Did you run db compare after update?');
